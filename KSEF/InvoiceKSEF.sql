...
-- warto eksportować też faktury z POS, więc dla nich zostaną dodane opcje maskujące/mapujące
--- synonim dla funkcji dbo.PozycjeDoWydruku aby raz pobierała dane z tabel szefa innym razem posowych
...
if (@application='POS') begin
	...
	-- załadowanie główki faktury którą skorygowaliśmy
...
if (@TypDokumentu<>'KOREK' and @application<>'POS') -- bo korekta ma już w sobie pozycje +/i i nie trzeba dodatkowo komensować innych dokumentów
begin
...
-- pozycje korekty muszą być odwrócone, najpierw pozycja korygowana na (-), później korygująca na (+)
-- dodatkowo dla nowych pozycji, musi być dodana również pozycja z ilością i wartością zero, jako pozycja korygowana
if (@TypDokumentu='KOREK') begin
	
	if (@debug=1) begin
	    exec dbo.ConvertKorekToKSEF @PozycjeFaktury
    end
	-- todo: tu warto się jeszcze przyjrzeć na różnych "konfiguracjach" korekty
	if (@application<>'POS') begin
...
if (@kodSystemowy=1) begin
	...
	(
	   select
				...
				,(	
					SELECT
						...
						,[PelnaNazwa] = case 
						 ...
						 end [ImiePierwsze] --tylko jeśli nie ma NIP-u (fizyczna)
						 ,case when nip.Ustawiono=0
							then case 
						 ...
						 end [Nazwisko] --tylko jeśli nie ma NIP-u (fizyczna)
					FOR XML PATH(''), ROOT('DaneIdentyfikacyjne'), TYPE
				)
				,(	
					SELECT
			...
			SELECT
				...
				,( -- Adnotacje
					select
						...
						,2 [P_18] --odwrotne obciążenie (1)
						,2 [P_18A] --convert(int,case when t.brutto>15000 then 1 else 2 end)  --mechanizm podzielonej płatności(1)
						,2 [P_19]
						,2 [P_22]
						,2 [P_23]
				...
				,( -- DodatkowyOpis -  w sumie są jedynie do Faktur Vat Marża
					select top 12 op.*
					from (
						select 
						'DocumentId' [Klucz]
				...
				,( -- FA(1) numery faktur zaliczkowych rozliczanych aktualną fakturą
				 	select 
		...
		FOR XML PATH('') --, ROOT('Faktura') odremować root i scheme przed selectem głónym
	)
end
	   ...
	   select
			...
			SELECT 
				 ( -- DaneIdentyfikacyjne
					SELECT
						...
						,[KodUE] = null
						,[NrVatUE] = null
						,[KodKraju] = null -- jesli nip zagraniczny, można uzupełnić
						,[NrID]=null -- jesli nip zagraniczny, można uzupełnić
						,[BrakID] = case when nip.Ustawiono=0 then 1 else null end											
		...
		,(
			SELECT
				...
				, case when nds.TypDokumentu='KOREK' 
					then (
						select 
							...
							,[P_15ZK] = null -- kwota faktury zaliczkowej przed korektą
							,[KursWalutyZK] = null 
						from @NaglowekFakturyKorygowanej kor
						FOR XML PATH(''), TYPE
						)
				...
				,( -- FakturaZaliczkowa --zaliczki są opakowane i prezentują numery ksef
				 	select 
						(
							select 
		...
		FOR XML PATH('') --, ROOT('Faktura') odremować root i scheme przed selectem głównym
	)
end
...
-- to poniżej nie konieczne, jeśli mamy ;WITH ze schemą przed selectem
set @cRequest = cast(
    '<Faktura '+
