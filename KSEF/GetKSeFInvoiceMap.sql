...
as begin
	...
	declare @towary_dostawy table (
		...
		,[IdMap] int default(0) --id mapowania, jeśli puste nie było na moment parsowania faktury KSeF
	)
	...
	-- aktualizacja o towary możiwe to zmapowania z kartoteką
	update @towary_dostawy set 
		...
		, [Zawartosc]=1
		, [IsLoad]=1
		--, [IDMagazynu]=tm.IDMagazynu, podmapować domyślny magazyn usera po SPID i sesji
	...
	begin catch
		print 'Błąd podczas importu faktury z KSeF'
		RAISERROR ('Błąd podczas importu faktury z KSeF', 16, 1) WITH NOWAIT, SETERROR
	end catch
end
go